...
public class Editor implements DisplayListener, MouseListener, MouseMotionListener, KeyListener {
	...
	public Editor (final Quantum quantum, final Gui gui) {
		...
		final ScreenAlignementContainer cont2 = new ScreenAlignementContainer(gui, HorizontalAlignement.LEFT,
	...
	public void display (GLCanvas canvas) {
		...
		for (int x = 0; x < 100; x++) {
			gl.glVertex2f(-50 * Constants.PLANET_MAX_SIZE * 4 + x * Constants.PLANET_MAX_SIZE * 4, -50 * Constants.PLANET_MAX_SIZE
				* 4);
			gl.glVertex2f(-50 * Constants.PLANET_MAX_SIZE * 4 + x * Constants.PLANET_MAX_SIZE * 4,
				50 * Constants.PLANET_MAX_SIZE * 4);
		}

		for (int x = 0; x < 100; x++) {
			gl.glVertex2f(-50 * Constants.PLANET_MAX_SIZE * 4, -50 * Constants.PLANET_MAX_SIZE * 4 + x * Constants.PLANET_MAX_SIZE
				* 4);
			gl.glVertex2f(+50 * Constants.PLANET_MAX_SIZE * 4, -50 * Constants.PLANET_MAX_SIZE * 4 + x * Constants.PLANET_MAX_SIZE
				* 4);
		}
	...
	public void mousePressed (MouseEvent e) {
		...
		if (wasGuiIntersected(e.getX(), e.getY())) return;
		...
		if (planet != null && selected_planets.contains(planet) == false) {
			if (ctrl_pressed == false) clearSelection();
	...
	public void mouseReleased (MouseEvent e) {
		...
		if (dragged) {
			if (drag_end != null) {
				...
				for (Planet planet : sim.getPlanets())
					if (planet.getPosition().x > x && planet.getPosition().x < x + width && planet.getPosition().y > y
						&& planet.getPosition().y < y + height) if (selected_planets.contains(planet) == false) {
						selected_planets.add(planet);
						createPlanetGui(planet);
					}
			}
	...
	public void mouseDragged (MouseEvent e) {
		...
		dragged = true;
		if (drag_start == null) {
			drag_start = new Vector2D(renderer.getCamera().getScreenToWorldX(e.getX()), renderer.getCamera().getScreenToWorldY(
				e.getY()));
		}

		if (button == MouseEvent.BUTTON1) {
			if (selected_planets.size() > 0) {
				Vector2D displacement = new Vector2D(renderer.getCamera().getScreenToWorldX(e.getX()), renderer.getCamera()
					.getScreenToWorldY(e.getY()));
				...
				drag_start.set(new Vector2D(renderer.getCamera().getScreenToWorldX(e.getX()), renderer.getCamera().getScreenToWorldY(
					e.getY())));
			} else {
				drag_end = new Vector2D(renderer.getCamera().getScreenToWorldX(e.getX()), renderer.getCamera().getScreenToWorldY(
					e.getY()));
			}
		}
	}
	...
	public void keyReleased (KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_CONTROL) ctrl_pressed = false;
