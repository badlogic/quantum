...
//		}
		...
		final ScreenAlignementContainer cont3 = new ScreenAlignementContainer( gui, HorizontalAlignement.RIGHT, VerticalAlignement.BOTTOM );
		...
		join.setClickedListener( new ClickedListener( ) {
			...
			{									
				...
				{					
					Log.println( "[JoinMenu] couldn't connect to server at " + ip.getText() + ":" + port_number + " :" + ex.getMessage() );
	...
	public void display( GLCanvas canvas) 
	{
		if( timer.getElapsedSeconds() > 3 )
		{
			...
			{				
				...
				if( irc != null )
				{
					if( irc.hasNewMessages( "#idle" ) )					
						if( !channel.equals( "#idle" ) )						
							idle.setForegroundColor(0, 1, 0, 1 );
					
					if( irc.hasNewMessages( "#quantum" ) )					
						if( !channel.equals( "#quantum" ) )						
							quant.setForegroundColor(0, 1, 0, 1 );
