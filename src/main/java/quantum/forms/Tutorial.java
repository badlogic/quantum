...
public class Tutorial implements DisplayListener {
	...
	enum TutorialState {
		Camera, Move, PlantTree, Statistics, Combat, Chain
	}
	...
	public Tutorial (final Quantum quantum, final Gui gui) {
		...
		dialog = new CustomDialog(gui, HorizontalAlignement.LEFT, VerticalAlignement.TOP, 300, "Tutorial - Camera Controls", label,
			back);
	...
	public void display (GLCanvas canvas) {
		...
		if (loop.getRenderer().getPlayerColor(2) == null) loop.getRenderer().allocatePlayerColor(sim, 2);

		if (state == TutorialState.Camera) doCamera();

		if (state == TutorialState.Move) doSelection();

		if (state == TutorialState.PlantTree) doPlantTree();

		if (state == TutorialState.Statistics) doStatistics();

		if (state == TutorialState.Combat) doCombat();

		if (state == TutorialState.Chain) doChain();
	}
	...
	private void doSelection () {
		...
		if (timer.isRunning() == false) {
			label.setText("");
			label.appendText("Good! Now that you know how to use " + "the camera let's move on to what "
				+ "you see in front of you.\n\n" + "By now you will have noticed the " + "red objects orbiting the circle. "
				+ "These objects are yours! We'll call " + "them creatures for the sake of simplicity. "
				+ "The circle is a planet that also belongs " + "to you. This is indicated by the color of "
				+ "creature depicted inside the planet\n\n" + "Creatures are your means to conquer "
				+ "new planets. To do so we have to send " + "them to other planets.\n\n"
				+ "Click on your home planet. You will see " + "lines indicating where you can send your "
				+ "creatures. Additionally you will see a " + "triangle on each line. If you click and drag "
				+ "that triangle you can specify how many of " + "your creatures should be moved to that planet.\n\n"
				+ "Send at least 3 of your creatures to the " + "planet on the left!");
	...
	private void doPlantTree () {
		if (timer.isRunning() == false) {
			label.setText("");
			label.appendText("Fine. The planet you just conquered " + "belongs to you now. This is again indicated "
				+ "by the color of the symbol in the middle of " + "the planet.\n\n"
				+ "To create new creatures you have to plant one " + "or more trees on one of your planets. A tree "
				+ "costs you 10 of your creatures orbiting the " + "planet. \n\nI just spawned 10 additional creatures "
				...
				+ "the planet. This symbol indicates that there's enough " + "creatures on that planet to build a tree.\n\n"
	...
	private void doStatistics () {
		if (timer.isRunning() == false) {
			label.setText("");
			label.appendText("You can see the tree grow now. It will grow, " + "produce a creature, grow two new branches, produce "
				+ "another two creatures and so on. To produce a creature "
				+ " the tree has to use up one resource of the planet. Planets "
				+ "have limited resources. This is indicated by their size, the " + "bigger a planet the more resources it has.\n\n"
				...
				+ "it. While the planet regrows resources your trees will wither down to " + "their roots.\n\n"
	...
	private void doChain () {
		if (timer.isRunning() == false) {
			...
			label.appendText("As it can be tedious to move creatures from planets "
				+ "to the battle front there's one more element called " + "chaining. If you chain one of your planets to another "
				+ " one of your planets creatures will automatically move "
				+ " from the source planet to the target planet. This can " + "relieve you from some micro managment.\n\n"
				...
				+ "button if you are comfortable with chaining to return to the main " + "menu");
			timer.start();
