...
public class UpdateMenu implements DisplayListener {
	...
	public UpdateMenu (final Quantum quantum, final Gui gui) {
		...
		try {
			...
			final ScreenAlignementContainer c = new ScreenAlignementContainer(gui, HorizontalAlignement.CENTER,
	...
	public void display (GLCanvas canvas) {
		if (updater != null) {
			if (updater.isUpdating()) {
				progress_label.setText("Updating, please wait ... " + updater.readSize() / 1024 + "kb / " + updater.totalSize()
					/ 1024 + "kb");
			}
			...
			if (updater.updateDone()) {
				...
				if (updater.updateFailed()) {
					...
					final ScreenAlignementContainer c = new ScreenAlignementContainer(gui, HorizontalAlignement.CENTER,
						VerticalAlignement.CENTER);
					ConfirmDialog dialog = new ConfirmDialog(gui, "Update successfull! Click OK to restart Quantum.", "Information",
						new ClickedListener() {
