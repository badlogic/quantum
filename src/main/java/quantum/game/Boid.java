...
public strictfp class Boid extends GameObject {
	...
	public void update () {
		if (behaviour == SteeringBehaviour.Seek) updateSeek();

		if (behaviour == SteeringBehaviour.Flee) updateFlee();

		if (behaviour == SteeringBehaviour.Arrive) updateArrive();

		if (behaviour == SteeringBehaviour.Circle) updateCircle();

		if (behaviour == SteeringBehaviour.Attack) updateAttack();

		if (behaviour == SteeringBehaviour.None) updateNone();

		if (force.len() > Constants.BOID_MAX_FORCE) force.nor().mul(Constants.BOID_MAX_FORCE);

		tmp.set(vel);

		vel.add(force);
		if (vel.len() > getMaxSpeed()) vel.nor().mul(getMaxSpeed());
	...
	protected void updateArrive () {
		if (pos.dst2(sim.getObject(target).pos) < 100) behaviour = SteeringBehaviour.None;
...
// }
	...
	protected void updateCircle () {
		...
		if (len2 < planet.getRadius() + Constants.BOID_MIN_ORBIT) {
			if (len2 < planet.getRadius() + Constants.BOID_MIN_ORBIT) vel.mul(0.93f);
	...
	protected void updateNone () {
		if (vel.len() < Constants.BOID_MIN_SPEED) return;
