...
public class Bot {
		...
		if (sim.getTurn() == sim.getNextCommandTurn()) {
			if (wait) {
				...
				try {
					...
					Log.println("[Bot] error calling update method of bot: " + e.getErrorSourceFile() + ":" + e.getErrorLineNumber()
						+ " - " + e.getErrorText());
				}
			}
		}
	...
	private void gatherConnectedPlanets (Simulation sim) {
		...
		for (Planet planet : sim.getPlanets()) {
			if (planet.getOwner() != -1 && planet.getOwner() != this.id) unvisited.add(planet);
		}
