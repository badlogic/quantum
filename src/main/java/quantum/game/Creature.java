...
public strictfp class Creature extends Boid implements AliveGameObject
{
	...
	public Creature(Simulation sim, int planet, int owner, float x, float y, float angle, float strength, float health, float speed) 
	{
		super(sim, x, y, angle);	
		this.planet = planet;
		this.owner = owner;
	...
	{
		return action == ACTION_ORBIT && sim.getPlanet(planet).getPosition().dst2(pos) < (sim.getPlanet(planet).getRadius() + Constants.BOID_MAX_ORBIT)*(sim.getPlanet(planet).getRadius() + Constants.BOID_MAX_ORBIT)?max_speed * 0.6f:max_speed;
	}
	...
	private void generateTriangles( )
	{
		...
		points[0] = p1.x; points[1] = p1.y;
		points[2] = p2.x; points[3] = p2.y;
		points[4] = h / 2 + Constants.BOID_SIZE * strength;
		
		points[6] = p1.x; points[7] = p1.y;
		points[8] = p3.x; points[9] = p3.y;
		points[10] = (p1.x - p3.x) / 2 - Constants.BOID_SIZE * speed;
		points[11] = (p1.y - p3.y) / 2 + Constants.BOID_SIZE * speed;
		
		points[12] = p2.x; points[13] = p2.y;
		points[14] = p3.x; points[15] = p3.y;
			...
			if( planet.hasEnemyCreatures() )
			{
				...
				if( target != null )
				{
					...
					if( target instanceof Creature )
					{
						...
						{
							if( sim.getAliveGameObject(((Creature)target).target) != null && sim.getAliveGameObject(((Creature)target).target).getPosition().dst2( target.getPosition() ) >
							    target.getPosition().dst2(pos) )
							{
								((Creature)target).action = ACTION_ATTACK;
								((Creature)target).steer( SteeringBehaviour.Attack, id );
							}
	...
	public AliveGameObject selectTarget( )
	{
		...
		if( nearest_target == null && could_attack == 0 )
		{
			...
			{
				if( tree.isDead() || planet.getOwner() == owner )
					continue;
