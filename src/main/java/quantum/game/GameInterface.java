...
public strictfp class GameInterface implements MouseListener, MouseMotionListener, KeyListener
{
	...
	class Handle
	{
		...
		{
			if( src.getFriendlyCreatures( loop.getClient().getPlayer().getId() ) == 0 )
				return false;
				...
				if( total_strength != 0 && dst_strength != 0 )
				{
					...
					if( dir.x < 0 && dir.y < 0 )	
					{
						gl.glBegin( GL.GL_LINES );						
							...
							gl.glVertex2f( (int)(x + dir.y * WIDTH * 1.5f)+ 100 * src_strength, (int)(y + dir.x * WIDTH * 1.5f)  - font.getHeight() * 1.5f - 3);
							gl.glColor3f( dst.getR(), dst.getG(), dst.getB() );
							gl.glVertex2f( (int)(x + dir.y * WIDTH * 1.5f)+ 100 * src_strength, (int)(y + dir.x * WIDTH * 1.5f)  - font.getHeight() * 1.5f - 3);
							gl.glVertex2f( (int)(x + dir.y * WIDTH * 1.5f)+ 100, (int)(y + dir.x * WIDTH * 1.5f)  - font.getHeight() * 1.5f - 3);
						gl.glEnd();
					}
					else
					{
	...
	private void renderDebugStats( )
	{
		...
		if( show_stats )
		{				
			font.renderTextNewLine( 10, canvas.getHeight(), 
					"\n\n\n\n\n\n\n\n\nturn: " + sim.getTurn() + "\ncommand turn length: " + sim.getCommandTurnLength() + "\nping: " + loop.ping.getLastPing() + " ms\nfps: " + renderer.getFramesPerSecond() + "\nobjects: " + sim.getObjectCount() + "\n" +
					"zoom: " + cam.getScale() + "\nsimulation: " + String.format( "%.2f", sim.getSimulationUpdateTime()) + "ms\n" +
					"rendering: " + String.format( "%.2f", renderer.getRenderTime( ) ) + "ms" + "\n" +
					"planet rendering: " + String.format( "%.2f", renderer.getPlanetRenderTime() ) + "ms" + "\n" +
					"tree rendering: " + String.format( "%.2f", renderer.getTreeRenderTime() ) + "ms" + "\n" +					
					"creature rendering: " + String.format( "%.2f", renderer.getCreatureRenderTime() ) + "ms" + "\n" +
					"glow rendering: " + String.format( "%.5f", renderer.getGlowRenderTime() ) + "ms" + "\n" +
					"gui rendering: " + String.format( "%.5f", renderer.getGuiRenderTime() ) + "ms" + "\n" +
					"culled: " + renderer.getCulledObjects() + " objects" + "\n" + 
					"mem: " + ( Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory() ) / 1024 + "/" + Runtime.getRuntime().totalMemory() / 1024 + " kb");
		}
	}
	...
	private void renderChain( Planet source, Planet target )
	{
		...
		tmp2.nor();
		tmp.set( planet.getPosition() ).add( tmp2.x * ( planet.getRadius() + Constants.BOID_MAX_ORBIT ),  tmp2.y * ( planet.getRadius() + Constants.BOID_MAX_ORBIT ) );
...
//		gl.glLineWidth( 1 );
	...
	private void renderPlayerStats( GLCanvas canvas )
	{
		...
		if( loop.getClient().getPlayerList() == null )
			return;
		...
		{	
			...
			if( clients.getIds().get(i) == loop.getClient().getPlayer().getId() && sim.getCreatureCount( clients.getIds().get(i) ) >= Constants.MAX_CREATURES )
			{
				mark.bind(0);
	...
	private void renderPlanetStats( GLCanvas canvas, Planet selected_planet )
	{
		...
		font.renderTextNewLine( font.getWidth( "resources " ) + a_x, a_y, "\n\n\n\n" +
				...
				+ selected_planet.getEnemeyCreatures( loop.getClient().getPlayer().getId() ) + "\n" + 
				 ...
				 + selected_planet.getResources() + "/" + selected_planet.getMaxResources() );
	...
	{	
		...
		{			
			...
			{							
				if( selected_planet != null && selected_planet.getMoveableCreatures( loop.getClient().getPlayer().getId() ) >= Constants.TREE_COST )
				{
		...
		else if (e.getButton() == MouseEvent.BUTTON2 || e.getButton() == MouseEvent.BUTTON3)
		{
			if( selected_planet != null && selected_planet.getOwner() == loop.getClient().getPlayer().getId() )
			{
				...
				if( ( target == null || target == selected_planet ) && selected_planet.getOwner() == loop.getClient().getPlayer().getId() )
				{
	...
	public void mouseReleased(MouseEvent e) 
	{
		if( selected_handle != null )
		{
			...
			{			
				...
				sim.moveCreatures( loop.getClient().getPlayer().getId(), selected_handle.src.getId(), selected_handle.dst.getId(), selected_handle.units );
			}
		}
	...
	{
		if( selected_handle != null )
			selected_handle.move( e.getX(), e.getY() );
	...
	{
		if( tree_button != null )
			tree_button.intersect( e.getX(), e.getY() );
