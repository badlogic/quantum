...
/**
 * class representing a simulation. responsible for buffering
 * commands and sending them out to the server on time as well
 * as executing received buffers. commands are buffered for
 * Constants.COMMAND_TURNS turns and then send to the server. 
 * When the server received all command buffers from all clients
 * for a command turn it sends the combined command buffers back
 * to each client where they get executed. Additionally the length
 * of a command turn is adapted with respect to the average largest
 * ping of a client. 
 * 
 * @author marzec
 *
...
public strictfp class Simulation 
{
	...
	/** 
	 * the number of turns added to the current command turn giving
	 * the next command turn will be executed 
	 **/
	private int cmd_turn_inc = Constants.COMMAND_TURNS;
	
	/**
	 * queue for received commandbuffer turns. the queue is 
	 * ordered by the turn id of the command buffers
	 */
	...
	/**
	 * constructor, sets the client to be used. in case client is null
	 * no commands will be buffered or send making this only a non-interactive
	 * simulation fed by enqueued buffers.
	 * @param client
	 */
	...
	/**
	 * sets the client to be used by this simulation. in case no 
	 * client is set this simulation will only be interpreting
	 * enqueued buffers. 
	 * 
	 * @param client
	 */
	...
	public Planet getPlanet( float x, float y )
	{
		...
		for( Planet planet: planets )	
			if( planet.getPosition().dst2(v) <= planet.getRadius() * planet.getRadius() )
				return planet;		
	...
	{		
		...
		{
			timer.start();
			if( processCommandBufferMessage() == false )
				return false;
	...
	/**
	 * checks wheter the current turn is a command buffer turn. if so
	 * it checks wheter an adequat commandbuffermessage is enqueue,
	 * removes it from the queue and executes it. in case no adequat
	 * command buffer is available it returns false indicating that
	 * no update will be executed. Also adjusts the command turn
	 * increment in accordance to the value set in the commandbuffermessage.
	 * 
	...
	protected boolean processCommandBufferMessage( ) throws Exception
	{
		if( turn == getNextCommandTurn() )
		{
			...
			{
				if( cmd_queue.peek() == null )
					return false;
				
				if( cmd_queue.peek().getCommandBuffer().getTurnId() != turn )			
					throw new Exception( "SEVERE ERROR!!! command buffer turn id invalid! current turn: " + turn + ", received turn: " + cmd_queue.peek().getCommandBuffer().getTurnId() );							
			...
			if( client != null )
			{
				...
				if( isGameOver() )
					client.sendMessage( new GameOverMessage() );
	...
	{	
		for( Command cmd: buffer )
		{
			if( cmd instanceof MoveCreatureCommand )
			{
				...
				if( !planet.isReachablePlanet( move_cmd.getDestinationPlanet() ) )
					continue;
				
				int send = 0;
				for( Creature creature: planet.getCreatures() )
				{
					if( creature.getOwner() == move_cmd.getId() && creature.isBorn() && creature.isDead() == false && (creature.isOrbiting() || creature.isAttacking() ) )
					{
						creature.move( move_cmd.getDestinationPlanet() );
						send++;
					}
					
					if( send == move_cmd.getUnits() )
						break;
				}
			}
			...
			if( cmd instanceof PlantTreeCommand )
			{
				...
				if( planet.getOwner() != plant_command.getId() )
					continue;
				
				if( planet.getFriendlyCreatures( planet.getOwner() ) < Constants.TREE_COST )
					continue;
				...
				for( Creature creature: planet.getCreatures() )
				{
					...
					if( removed == Constants.TREE_COST )
						break;
				}
			}
			...
			if( cmd instanceof ChainCommand )
			{
				...
				if( source.isReachablePlanet( target.getId() ) && 
					target.getChainedPlanet() != source.getId() )
				{
	...
	/**
	 * enqueues the commandbuffermessage for execution when its
	 * turn is reached. 
	 * @param buffer
	 */
	...
	/**
	 * writes the complete simulation state to the output stream.
	 * can be read in by the readState method. 
	 * @param out
	 * @throws IOException
	 */
	...
	{	
		...
		{					
			...
			for( int key: player_stats.keySet() )
			{
				...
				{
					for( Creature creature: planet.getCreatures() )
						if( creature.getOwner() == key )
							creatures++;
					
					if( planet.getOwner() == key )
						trees += planet.getTrees().size();
				}							
				
				if( creatures != 0 || trees != 0 )
					active++;
			}					
