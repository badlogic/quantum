		...
		public void update () {
			...
			if (curr_height < height && grown_creatures == 0) {
				...
				if (grown_creatures < Constants.TREE_MAX_CREATURES || depth == Constants.TREE_DEPTH) {
					...
					if (children[0] == null) {
						children[0] = new Branch(sim, tree, planet, pos.cpy().add(dir.cpy().mul(curr_height)), angle
							+ Constants.TREE_ANGLE * sim.rand(0.5f, 1), Constants.TREE_DELATION * height * sim.rand(0.5f, 1.5f),
							depth + 1);
						children[1] = new Branch(sim, tree, planet, pos.cpy().add(dir.cpy().mul(curr_height)), angle
							- Constants.TREE_ANGLE * sim.rand(0.5f, 1), Constants.TREE_DELATION * height * sim.rand(0.5f, 1.5f),
							depth + 1);
					} else {
						children[0].update();
