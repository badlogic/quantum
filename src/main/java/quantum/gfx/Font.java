...
public class Font {
	...
	/** creates a new font with the given face, size and style OpenGL has to be intialized already at this point
	...
	protected int getJavaFontStyle (FontStyle style) {
		if (style == FontStyle.Plain) return java.awt.Font.PLAIN;
		if (style == FontStyle.Bold) return java.awt.Font.BOLD;
		if (style == FontStyle.Italic) return java.awt.Font.ITALIC;
		if (style == FontStyle.BoldItalic) return java.awt.Font.BOLD | java.awt.Font.ITALIC;

		return java.awt.Font.PLAIN;
	}

	protected BufferedImage getGlyphImage (java.awt.Font font, int img_size, HashMap<Character, TextureTile> tiles,
		HashMap<Character, Integer> advances) {
	...
	/** renders the given text. texture unit 0 is used for the texture map and not reset to the previous binding. no additional care
	 * is taken for escape sequences or new lines and the like. alpha testing is enabled and reset, but not the alpha function (
	 * glAlphaFunc( GL_GREATER, 0.1f ) is used ). the text is placed on the local x/y plane, were the current translation specifies
	 * the upper left corner of the line of text.
	 * 
	...
	public void renderText (String text) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if (!tiles.containsKey(c)) continue;
	...
	/** pushes the modelview matrix, translates it by (x,y) and renders the given text
	 * 
	 * @param x
	 * @param y
	 * @param text */
	...
	/** renders the given text starting at x,y interpreting new line. y is decreased for each line.
	 * @param x
	 * @param y
	 * @param text */
