...
public class OrthoCamera implements MouseListener, MouseMotionListener, MouseWheelListener, KeyListener {
	...
	public void update (int width, int height) {
		...
		model.idt();
		model.setToTranslation(new Vector((float)(-pos.getX() + (width / 2) * scale), (float)(-pos.getY() + (height / 2) * scale),
			(float)(-pos.getZ())));
		combined.set(proj);
		combined.mul(model);
	...
	public void update (GLCanvas canvas) {
		...
		model.idt();
		model.setToTranslation(new Vector((float)(-pos.getX() + (canvas.getWidth() / 2) * scale), (float)(-pos.getY() + (canvas
			.getHeight() / 2) * scale), (float)(-pos.getZ())));
		combined.set(proj);
		combined.mul(model);
	...
	public void update (GLCanvas canvas, boolean update_matrices) {
		...
		if (!interpolating) {
			...
			if (input_disabled == false) {
				...
				zoom_speed *= 0.93f;
				if (Math.abs(zoom_speed) < 0.00001) zoom_speed = 0;
			}
	...
	/** returns wheter the given button is pressed. use the MouseEvent.BUTTONX enumeration for specifying the button.
	 * @param button
	 * @return */
	...
	/** returns the mouse position relative to the gl canvas
	 * @return */
