...
public class Renderer {
	...
	public Renderer () {
		try {
			...
			vert_shader = new Shader(FileManager.readFile("shader/vertex.glsl"),
				FileManager.readFile("shader/verticalgaussian.glsl"));
			hort_shader = new Shader(FileManager.readFile("shader/vertex.glsl"),
				FileManager.readFile("shader/horizontalgaussian.glsl"));
	...
	public void useInstancing (boolean value) {
		instancing = value;
		if (instance_shader == null) instancing = false;
	}
	...
	public void renderPass (Simulation sim, GameInterface gui, GLCanvas canvas) {
		...
		for (Planet planet : sim.getPlanets()) {
			...
			if (planet == selected_planet) renderPlanetPaths(canvas, sim, selected_planet);

			if (render_all_paths) renderPlanetPaths(canvas, sim, planet);

			if (render_is_start_planet) {
				if (planet.isStartPlanet()) planet.renderMesh(canvas, 0.2f, 0, 1, 0);
			}
