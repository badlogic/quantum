...
public class Button extends Widget {
	...
	public void keyPressed (int key_code) {
		if (key_code == KeyEvent.VK_ENTER || key_code == KeyEvent.VK_SPACE) pressed = true;
	}
	...
	public void mouseReleased (float x, float y, int button) {
		pressed = false;
		if (listener != null && internalIntersect(x, y)) listener.clicked(this);
	...
	protected void renderPressed () {
		...
		if (caption != null)
			font.renderText((int)(pos.x + width / 2 - text_width / 2 + 2),
				(int)(pos.y - height / 2 + font.getHeight() / 2 - 2 + font.getDescent()), caption);
		gui.disableScissor();
	}
	...
	protected void renderReleased () {
		...
		if (caption != null)
			font.renderText((int)(pos.x + width / 2 - text_width / 2),
				(int)(pos.y - height / 2 + font.getHeight() / 2 + font.getDescent()), caption);
		gui.disableScissor();
	}
