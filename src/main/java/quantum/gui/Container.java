...
public class Container extends Widget {
	...
	private Widget getWidget (float x, float y) {
		for (Widget widget : widgets) {
			if (widget.isVisible() == false) continue;

			if (widget.intersects(x, height + y)) return widget;
		}
	...
	public void mouseDragged (float x, float y, int button) {
		...
		last_pressed_widget.mouseDragged(x - last_pressed_widget.getPosition().x, height + y - last_pressed_widget.getPosition().y,
			button);
	}
	...
	public void mouseReleased (float x, float y, int button) {
		...
		last_pressed_widget.mouseReleased(x - last_pressed_widget.getPosition().x,
			height + y - last_pressed_widget.getPosition().y, button);
	...
	protected void layout () {
		for (Widget widget : getWidgets()) {
			if (widget instanceof Container) if (widget.isVisible()) ((Container)widget).layout();
		}
	}
