...
public class HorizontalBoxContainer extends Container {
	...
	private float getMaxHeight () {
		...
		for (Widget widget : getWidgets()) {
			if (widget instanceof Container) ((Container)widget).layout();
	...
	protected void layout () {
		...
		for (int i = 0; i < getWidgets().size(); i++) {
			...
			if (v_align == VerticalAlignement.TOP) widget.setY(max_height);

			if (v_align == VerticalAlignement.CENTER) widget.setY(max_height / 2 + widget.getHeight() / 2);

			if (v_align == VerticalAlignement.BOTTOM) widget.setY(widget.getHeight());
		}
