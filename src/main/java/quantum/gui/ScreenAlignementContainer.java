...
public class ScreenAlignementContainer extends Container {
	...
	public void layout () {
		...
		if (h_align == HorizontalAlignement.LEFT) pos.x = 0;
		if (h_align == HorizontalAlignement.CENTER) pos.x = gui.getCanvas().getWidth() / 2 - width / 2;
		if (h_align == HorizontalAlignement.RIGHT) pos.x = gui.getCanvas().getWidth() - width;

		if (v_align == VerticalAlignement.TOP) pos.y = gui.getCanvas().getHeight();
		if (v_align == VerticalAlignement.CENTER) pos.y = gui.getCanvas().getHeight() / 2 + height / 2;
		if (v_align == VerticalAlignement.BOTTOM) pos.y = height;
