...
public class TextField extends Widget {
	...
	public void keyTyped (char character) {
		...
		if (character != '\b' && character != '\n' && Character.isISOControl(character)) return;

		if (character == '\n') {
			if (listener != null) listener.pressedEnter(this);
			return;
		}
	...
	public void render (GLCanvas canvas) {
		...
		if (alignement == Alignement.LEFT) x = pos.x + 2;

		if (alignement == Alignement.CENTER) x = pos.x + (width - 4) / 2 - font.getWidth(text) / 2;

		if (alignement == Alignement.RIGHT) x = pos.x + width - font.getWidth(text);
		...
		if (blink_timer.getElapsedSeconds() > 0.5) {
			...
			if (!hasFocus()) blink = false;
		}

		gui.enableScissor((int)pos.x + 1, (int)pos.y - (int)height, (int)width - 2, (int)height);

		if (x + font.getWidth(text + "_") > pos.x + width) x = x - (x + font.getWidth(text + "_") - (pos.x + width));
