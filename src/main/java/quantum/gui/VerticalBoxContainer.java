...
public class VerticalBoxContainer extends Container {
	...
	protected void layout () {
		...
		for (int i = 0; i < getWidgets().size(); i++) {
			...
			if (h_align == HorizontalAlignement.CENTER) widget.setX(max_width / 2 - widget.getWidth() / 2);

			if (h_align == HorizontalAlignement.RIGHT) widget.setX(max_width - widget.getWidth());

			if (h_align == HorizontalAlignement.LEFT) widget.setX(0);
		}
