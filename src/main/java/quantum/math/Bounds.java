...
public final class Bounds implements Serializable {
	...
	/** returns the corner points of this bounds. returns the back and front rectangles in counterclockwise order starting at the
	 * bottom left, front is minz back is maxz
	 * @return */
	...
	public synchronized Bounds set (Vector a_min, Vector a_max) {
		min.set(a_min.getX() < a_max.getX() ? a_min.getX() : a_max.getX(),
			a_min.getY() < a_max.getY() ? a_min.getY() : a_max.getY(), a_min.getZ() < a_max.getZ() ? a_min.getZ() : a_max.getZ());
	...
	public synchronized Bounds ext (float[] a_vec) {
		crn_dirty = true;
		return this.set(min.set(Math.min(min.getX(), a_vec[0]), Math.min(min.getY(), a_vec[1]), Math.min(min.getZ(), a_vec[2])),
			max.set(Math.max(max.getX(), a_vec[0]), Math.max(max.getY(), a_vec[1]), Math.max(max.getZ(), a_vec[2])));
	}

	public synchronized Bounds ext (Vector a_vec) {
		crn_dirty = true;
		return this.set(
			min.set(Math.min(min.getX(), a_vec.getX()), Math.min(min.getY(), a_vec.getY()), Math.min(min.getZ(), a_vec.getZ())),
			max.set(Math.max(max.getX(), a_vec.getX()), Math.max(max.getY(), a_vec.getY()), Math.max(max.getZ(), a_vec.getZ())));
	}
	...
	public synchronized Bounds add (Bounds a_bounds) {
		crn_dirty = true;
		return this.set(
			min.set(Math.min(min.getX(), a_bounds.min.getX()), Math.min(min.getY(), a_bounds.min.getY()),
				Math.min(min.getZ(), a_bounds.min.getZ())),
			max.set(Math.max(max.getX(), a_bounds.max.getX()), Math.max(max.getY(), a_bounds.max.getY()),
	...
	public synchronized Bounds mul (Matrix a_matrix) {
		...
		for (Vector l_pnt : crn) {
			l_pnt.mul(a_matrix);
			min.set(Math.min(min.getX(), l_pnt.getX()), Math.min(min.getY(), l_pnt.getY()), Math.min(min.getZ(), l_pnt.getZ()));
			max.set(Math.max(max.getX(), l_pnt.getX()), Math.max(max.getY(), l_pnt.getY()), Math.max(max.getZ(), l_pnt.getZ()));
		}
		crn_dirty = true;
	...
	public synchronized boolean contains (Vector v) {
		if (min.getX() > v.getX()) return false;
		if (max.getX() < v.getX()) return false;
		if (min.getY() > v.getY()) return false;
		if (max.getY() < v.getY()) return false;
		if (min.getZ() > v.getZ()) return false;
		if (max.getZ() < v.getZ()) return false;
