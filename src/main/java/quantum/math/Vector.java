...
public final class Vector implements Serializable {
	...
	public float dst (Vector a_vec) {
		return (float)(Math.sqrt(Math.pow(a_vec.val[0] - val[0], 2) + Math.pow(a_vec.val[1] - val[1], 2)
			+ Math.pow(a_vec.val[2] - val[2], 2)));
	}

	public float sqrdist (Vector a_vec) {
		return (float)(Math.pow(a_vec.val[0] - val[0], 2) + Math.pow(a_vec.val[1] - val[1], 2) + Math.pow(a_vec.val[2] - val[2], 2));
	}
	...
	public Vector crs (Vector a_vec) {
		return this.set(val[1] * a_vec.val[2] - val[2] * a_vec.val[1], val[2] * a_vec.val[0] - val[0] * a_vec.val[2], val[0]
			* a_vec.val[1] - val[1] * a_vec.val[0]);
	}
	...
	public Vector mul (Matrix a_matrix) {
		...
		return this.set(val[0] * l_mat[Matrix.M00] + val[1] * l_mat[Matrix.M01] + val[2] * l_mat[Matrix.M02] + l_mat[Matrix.M03],
			val[0] * l_mat[Matrix.M10] + val[1] * l_mat[Matrix.M11] + val[2] * l_mat[Matrix.M12] + l_mat[Matrix.M13], val[0]
				* l_mat[Matrix.M20] + val[1] * l_mat[Matrix.M21] + val[2] * l_mat[Matrix.M22] + l_mat[Matrix.M23]);
	}
	...
	public Vector prj (Matrix a_matrix) {
		...
		return this.set((val[0] * l_mat[Matrix.M00] + val[1] * l_mat[Matrix.M01] + val[2] * l_mat[Matrix.M02] + l_mat[Matrix.M03])
			/ l_w, (val[0] * l_mat[Matrix.M10] + val[1] * l_mat[Matrix.M11] + val[2] * l_mat[Matrix.M12] + l_mat[Matrix.M13]) / l_w,
	...
	public Vector rot (Matrix a_matrix) {
		float l_mat[] = a_matrix.val;
		return this.set(val[0] * l_mat[Matrix.M00] + val[1] * l_mat[Matrix.M01] + val[2] * l_mat[Matrix.M02], val[0]
			* l_mat[Matrix.M10] + val[1] * l_mat[Matrix.M11] + val[2] * l_mat[Matrix.M12], val[0] * l_mat[Matrix.M20] + val[1]
			* l_mat[Matrix.M21] + val[2] * l_mat[Matrix.M22]);
	}

	public Vector min (Vector a_vector) {
		return this.set(a_vector.getX() < getX() ? a_vector.getX() : getX(), a_vector.getY() < getY() ? a_vector.getY() : getY(),
