...
public strictfp class Client {
	...
	public Client (String name, String ip, int port) throws Exception {
		...
		if (v_msg.getVersion() != Constants.VERSION) throw new RuntimeException("Version mismatch");
	...
	public Message readMessage () throws Exception {
		...
		if (msg instanceof PlayerListMessage) player_list = (PlayerListMessage)msg;

		if (msg instanceof TextMessage) log.add(((TextMessage)msg).getName() + ": " + ((TextMessage)msg).getMessage());

		if (msg instanceof DisconnectedMessage) log.add(((DisconnectedMessage)msg).getName() + ": disconnected");
