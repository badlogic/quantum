...
public class IRC {
	private class Channel {
		...
		public void removeUser (String user) {
			...
			while (iter.hasNext())
				if (iter.next().equals(user)) iter.remove();
		}
	...
	public void sendText (String receiver, String sender, String msg) throws Exception {
		synchronized (this) {
			if (channels.containsKey(receiver)) channels.get(receiver).addMessageExclusive(sender + ": " + msg);
	...
	public void dispose () {
		try {
			...
			if (thread_recv != null) thread_recv.stop();
		} catch (IOException e) {
		}
