...
public strictfp class Server 
{
	...
	/**
	 * Handshake Thread. Waits for incoming Connections, negotiates
	 * the id and retrieves the name of the Client. If everything
	 * went ok the client is passed to the server.
	 * 
	 * @author marzec
	 *
	 */
	...
	/**
	 * the loby thread, checks for incoming messages and ready signals.
	 * kicks of the game thread in case all clients are ready.
	 * @author marzec
	 *
	 */
				...
				{							
						...
						try 
						{
							...
							if( msg != null )
							{
								...
								if( msg instanceof ReadyMessage )
								{
									client.ready = true;
									broadcastMessage( msg );
									Log.println( "[Server] client " + client.getPlayer().getId() + "(" + client.getPlayer().getId() + ") is ready" );
								}
								...
								if( msg instanceof MapImageMessage )									
								{
									try
									{
										...
										Simulation sim = new Simulation( false );
										DataInputStream in = new DataInputStream( new FileInputStream( FileManager.getPath() + "dat/maps/" + image_msg.getName() ) );
										sim.readState( in );									
										in.close();
					...
					if( udp_timer.getElapsedSeconds() > 4 )
					{
						try 
						{
							...
							String ip = (0x000000ff & ((int)addr.getAddress()[0])) + "." + 
										(0x000000ff & ((int)addr.getAddress()[1])) + "." +
										(0x000000ff & ((int)addr.getAddress()[2])) + "." +
										(0x000000ff & ((int)addr.getAddress()[3]));
	...
	/**
	 * The game thread. handles turn command buffers and schedules them
	 * for broadcasting adequadly
	 * @author marzec
	 *
	 */
	strictfp class Game implements Runnable
	{
						...
						{					
							...
							if( msg != null )
							{
								...
								{									
									...
									{
										Log.println( "[Server] received buffer for unexpected turn " + cmd_msg.getCommandBuffer().getTurnId() + " from client " + client.getPlayer().getName() );										
										System.exit(0);
									}
							...
							{
								if( last_received_client.get( client ).getElapsedSeconds() > 5 )
									disconnected.add( client );
							}
						} 
						catch (Exception e) 
		...
		{		
			...
			{ 
				...
				try {
					DataInputStream dataInputStream = new DataInputStream( new FileInputStream( FileManager.getPath() + "dat/maps/" + map_file ));
	...
	public void sendPlayerList( Client c )
	{
		synchronized( clients )
		{
			...
			try {
				...
				Log.println( "[Server] couldn't send PlayerListMessage to client " +c.getPlayer().getName() + ": " + Log.getStackTrace( e ));
				e.printStackTrace();
			}
		}
	}	
					...
					for( Player player: disconnected_client.getPlayers() )
					{
						...
						for( Client client: clients )
						{
							...
							try {
								...
								Log.println( "[Server] disconnected player: " + client.getPlayer().getName() + ", " + client.getPlayer().getId() );
							}
						}
					}
				}
