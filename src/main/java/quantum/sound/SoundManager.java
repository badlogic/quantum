...
/** a simple sound manager based on the vorbisspi by javazoom. This thing is a fake singleton executing asynchronously in a thread
 * which is automatically created and destroyed on program exit.
 * 
 * @author marzec */
public class SoundManager {
	...
	private static void update () {
		...
		synchronized (streams) {
			...
			if (audio_line.available() == 0) return;
...
// Log.println( "available: " + audio_line.available() + "/" + audio_line.getBufferSize() + ", read: " + read );
	...
	public static SoundStream playStream (String file) {
		if (audio_line == null) return null;
	...
	public static void playBuffer (String handle) {
		if (!buffers.containsKey(handle)) return;
