...
public class FileManager {
	...
	private static void loadMapsAndBots () {
		try {
			...
			while ((line = in.readLine()) != null) {
				if (line.startsWith("<") || line.equals("") || line.contains(".map") == false) continue;
		...
		try {
			...
			String line = null;
			while ((line = in.readLine()) != null) {
				if (line.startsWith("<") || line.equals("") || line.contains(".bsh") == false) continue;
	...
	public static void uploadMap (String map, String remote_map) throws Exception {
		try {
			...
			String command = "--dill\r\n" + "Content-Disposition: form-data; name=\"uploadfile\"; filename=\"" + remote_map
			...
			String header = "POST " + path + " HTTP/1.0\r\n" + "Accept: */*\r\n" + "Referer: http://localhost\r\n"
				...
				+ "Host: localhost\r\n" + "Content-Length: " + ((int)file.length() + command.length() + trail.length()) + "\r\n"
