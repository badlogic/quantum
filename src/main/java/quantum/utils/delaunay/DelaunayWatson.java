...
/** DelaunayWatson represents an O(N^2) method with low overhead to find the Delaunay triangulation or tetrahedralization of a set
 * of samples of R^2 or R^3.
 * <P> */
			...
			for (i1 = 0; i1 <= i7; i1++) {
				...
				if (dim < 3) {
					...
					ccr[id[i4]][0] = ((wrk[0][3] * (wrk[1][1] * wrk[2][2] - wrk[2][1] * wrk[1][2]))
						- (wrk[0][1] * (wrk[1][3] * wrk[2][2] - wrk[2][3] * wrk[1][2])) + (wrk[0][2] * (wrk[1][3] * wrk[2][1] - wrk[2][3]
						* wrk[1][1])))
						/ xx;
					ccr[id[i4]][1] = ((wrk[0][0] * (wrk[1][3] * wrk[2][2] - wrk[2][3] * wrk[1][2]))
						- (wrk[0][3] * (wrk[1][0] * wrk[2][2] - wrk[2][0] * wrk[1][2])) + (wrk[0][2] * (wrk[1][0] * wrk[2][3] - wrk[2][0]
						* wrk[1][3])))
						/ xx;
					ccr[id[i4]][2] = ((wrk[0][0] * (wrk[1][1] * wrk[2][3] - wrk[2][1] * wrk[1][3]))
						- (wrk[0][1] * (wrk[1][0] * wrk[2][3] - wrk[2][0] * wrk[1][3])) + (wrk[0][3] * (wrk[1][0] * wrk[2][1] - wrk[2][0]
						* wrk[1][1])))
						/ xx;
				}
				for (ccr[id[i4]][dim] = 0, i2 = 0; i2 < dim; i2++) {
					ccr[id[i4]][dim] += (pts[i0][i2] - ccr[id[i4]][i2]) * (pts[i0][i2] - ccr[id[i4]][i2]);
...
/*
 * OUTPUT is in a3s ARRAY needed output is: Tri - array of pointers from triangles or tetrahedra to their corresponding vertices
 * Vertices - array of pointers from vertices to their corresponding triangles or tetrahedra Walk - array of pointers from
 * triangles or tetrahedra to neighboring triangles or tetrahedra Edges - array of pointers from each triangle or tetrahedron's
 * edges to their corresponding triangles or tetrahedra
 * 
...
/*
 * DO NOT DELETE THIS COMMENTED CODE IT CONTAINS ALGORITHM DETAILS NOT CAST INTO JAVA (YET?) i0 = -1; for (i11=0; i11<nts; i11++)
 * { i0++; while (a3s[i0][0] < 0) i0++; if (a3s[i0][0] < nrs) { for (i1=0; i1<dim; i1++) for (i2=0; i2<dim; i2++) { wrk[i1][i2] =
 * pts[a3s[i0][i1]][i2] - pts[a3s[i0][dim]][i2]; } if (dim < 3) { xx = wrk[0][0] * wrk[1][1] - wrk[0][1] * wrk[1][0]; if (fabs(xx)
 * > EPSILON) { if (xx < 0) fprintf(afile,"%d %d %d\n",a3s[i0][0],a3s[i0][2],a3s[i0][1]); else
 * fprintf(afile,"%d %d %d\n",a3s[i0][0],a3s[i0][1],a3s[i0][2]); fprintf(bfile,"%e %e %e\n",ccr[i0][0],ccr[i0][1],ccr[i0][2]); } }
 * else { xx = ((wrk[0][0] * (wrk[1][1] * wrk[2][2] - wrk[2][1] * wrk[1][2])) - (wrk[0][1] * (wrk[1][0] * wrk[2][2] - wrk[2][0] *
 * wrk[1][2])) + (wrk[0][2] * (wrk[1][0] * wrk[2][1] - wrk[2][0] * wrk[1][1]))); if (fabs(xx) > EPSILON) { if (xx < 0)
